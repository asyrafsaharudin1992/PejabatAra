...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    // Find user row
    const getUrl = `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/Users!A:D`;
    const getRes = await fetch(getUrl, { headers: { Authorization: `Bearer ${token}` } });
    const getData = await getRes.json();
    const rows = getData.values || [];
    const rowIndex = rows.findIndex((row: string[]) => row[0]?.toLowerCase() === email?.toLowerCase());

    if (rowIndex !== -1) {
      const storedPassword = rows[rowIndex][3];
      if (storedPassword !== currentPassword) {
        return res.status(401).json({ error: "Current password incorrect" });
      }
      
      // Update Password (Col D = index 3)
